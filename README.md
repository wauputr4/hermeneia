...
- `hermeneia schedules` lists scheduled publishing records, artifact IDs, and statuses, with optional `--run`, `--status`, `--platform`, `--from`, and `--to` filters plus `--json` for automation-safe structured output including validation metadata.
