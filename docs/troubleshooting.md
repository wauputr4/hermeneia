...
- The Web UI agenda is read-only and loads `GET /v1/scheduled-posts`
  ...
  SQLite database.
- `GET /v1/scheduled-posts` accepts optional `run_id`, `artifact_id`, `status`,
  `platform`, `from`, and `to` query filters for API-side agenda narrowing. Use supported
  ...
  `run_id` or `artifact_id` values return an empty scheduled-post list instead
  of an error.
- The Web UI agenda displays schedule validation metadata when available. If
  ...
  outside SQLite.
- The Web UI agenda sends the active run/status/platform filters to
...
- Use `hermeneia cancel-schedule <schedule-id>` or
  ...
  review state.
- The CLI `hermeneia schedules` command accepts `--run`, `--artifact`,
  `--status`, `--platform`, `--from`, and `--to` filters. Invalid statuses,
  unsupported platforms, invalid RFC3339 range values, or inverted ranges
  return a clear non-zero error; omit a flag to leave that dimension unfiltered.
  Unknown run or artifact IDs return the normal empty schedule result. The human
  table includes an `ARTIFACT ID` column and prints `none` for schedules that
  were created without a selected artifact.
